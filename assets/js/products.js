var lastRenderedGrid = 0,filteredObject, allObjects
var categories = []
var allCategories = []
var sort = "none"
var sortDesc
      ...
      allObjects.map(function(product) {
          ...
          if ( categories.indexOf(category) == -1 ) {
            categories.push(category)
             ...
             $('<label />', { text: capitalizeString(category) }).add("<br />").appendTo($('.filter'))
          }
      })
...
function renderContentGrid(index) {
  ...
  $('#temp span').html("Score% : " + filteredObject[index].score*100)
  return $("#temp").html()
}
